...
public:
    ...
    void kruskall(string arquivoSaida, bool s, bool o)
    {
    ...
    for (int i = 1; i < argc; i++)
    {
        if (!strcmp(argv[i], "-h"))
            cout << "-h : mostra o help\n"
                    ...
                    "custo = ./kruskal.bin -f <arquivo>\n"
                    "solucao = ./kruskal.bin -f <arquivo> -s\n"
                    "inserir em um arquivo = ./kruskal.bin -f <arquivo> -o <arquivo>\n"
                 << endl;
        if (!strcmp(argv[i], "-s"))
        {
            s = true;
        ...
        if (!strcmp(argv[i], "-o")) // entrada -o
        {

            // arquivo de saida depois dele
            if (argv[i + 1] == NULL)
            {
...
/// problemas notados:
// 1 - em qualquer exmeplo ou digitar o -o ele da segmentation fault
// 2 - se nao digitar o -o ele da segmentation fault no man mans no exemp nao da
